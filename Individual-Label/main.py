...
class SetupWindow(QWidget):
    def __init__(self):
        ...
        self.labels_inputs_description = QLabel('b) or specify how many unique labels you want to assign. (note - you can assign more than one label to a single image.)', self)
...
class LabelerWindow(QWidget):
    def __init__(self, labels, input_folder, mode):
        ...
        self.show_next_checkbox = QCheckBox("Automatically show next image when labeled. (will prevent assigning multiple labels to single image)", self)
    ...
    def generate_csv(self, out_filename):
        ...
        """
        path_to_save = os.path.join(self.input_folder, 'Labeled-Output')
